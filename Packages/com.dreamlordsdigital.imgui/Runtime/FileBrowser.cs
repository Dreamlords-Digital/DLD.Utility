...
public partial class FileBrowser
{
	...
	public FileBrowser(
		string initialPath, string filePattern,
...
#if UNITY_EDITOR
			Object[] subAssets =
				AssetDatabase.LoadAllAssetRepresentationsAtPath(FileUtil.GetPathRelativeToAssets(animationFullPath));
#else
	...
	public void SwitchCurrentPath(string fullPath)
	{
		...
		if (enteredFbxIdx > 0 || enteredAssetBundleIdx > 0)
		{
			if (enteredAssetBundleIdx > 0)
			{
				...
				if (enteredFbxIdx > 0)
				{
					...
					{
						Debug.LogError($"Shouldn't be entering inside fbx files if not browsing animations. Path: \"{fullPath}\"");
					}
				}
				else
				{
			...
			else if (enteredFbxIdx > 0)
			{
				...
				if (IsBrowserTypeForAnimations)
				{
					...
					if (IsAnimationVirtualFolderAnFbxFile(fullPath, virtualFolderSpecified))
					{
						...
						{
							Debug.LogError($"FileBrowser.SwitchCurrentPath(): Can't open during runtime: \"{virtualFolderSpecifiedAbsPath}\"");
							return;
						}
					...
					{
						Debug.LogError($"FileBrowser.SwitchCurrentPath(): Can't open: \"{virtualFolderSpecifiedAbsPath}\"");
						return;
					}
				}
				else
			...
			else
			{
				...
				Debug.LogError($"FileBrowser.SwitchCurrentPath(): Unknown virtual folder in specified path: \"{fullPath}\"");
		...
		if (!string.IsNullOrEmpty(virtualFolderSpecified))
		{
			...
			{
				Debug.LogWarning($"Can't access file (perhaps it was renamed or deleted?): {virtualFolderSpecifiedAbsPath}");
	...
	protected void ReadDirectoryContents(
		string newPathToRead, string virtualFolder, string virtualPath,
		string previousPath)
	{
				...
				if (!string.IsNullOrEmpty(virtualFolder))
				{
					if (isVirtualFolderAnFbxFile)
					{
						...
						else
						{
							...
							if (subAssets == null || subAssets.Length == 0)
							{
								Debug.LogError($"FileBrowser.ReadDirectoryContents(): no sub-assets inside of \"{newPathToRead}\"");
								return;
							}
							...
							{
								Debug.LogWarning($"FileBrowser.ReadDirectoryContents(): no animation clips inside of \"{newPathToRead}\"");
							}
						}
					}
					else if (IsFilenameAnAssetBundle(virtualFolder))
	...
	void LoadImagesInCurrentPath(
		int startIdx = 0, int len = DEFAULT_NUMBER_OF_IMAGES_TO_AUTO_LOAD,
		bool forceReload = false)
	{
		...
		Debug.Log($"Loading images from {startIdx.ToString()}, length: {imagesToAutoLoad.ToString()} final idx: {(startIdx + imagesToAutoLoad - 1).ToString()}");
		...
		for (int i = startIdx; i < startIdx + imagesToAutoLoad; ++i)
		{
			...
			EditorCoroutineUtility.StartCoroutine(DownloadImage(fileUrl, fileAbsolutePath, i), this);
#else
	...
	protected void BuildContent()
	{
		...
		{
			Debug.Log($"FileBrowser.BuildContent(): making use of _pathUsed.Path: {_pathUsed.Path}. SelectedFileIdx: {_pathUsed.SelectedFileIdx}");
	...
	public void SetStyles(
		string folderRootButtonStyle, string folderButtonStyle, string listItemStyle,
			...
			if (newHoveredFileIdx != _hoveredFileIdx)
			{
				...
				if (_adjustedMouseY >= 0 && _adjustedMouseY <= _scrollViewRect.height - SCROLLBAR_SPACE)
				{
					...
					if (IsBrowserTypeForImages && DoesFileHaveImageThumbnail(_hoveredFileIdx))
					{
						...
						var fileSize =
							FileUtil.GetBytesReadable(FileUtil.GetFileSizeInBytes($"{_currentPath}/{_files[_hoveredFileIdx]}"));
...
Screen.height: {
	...
	void PingSelectedFile(int selectedFileIdx)
	{
		...
		if (!_currentPath.Contains(Application.dataPath))
		{
			...
			Debug.Log($"Will not ping because current path is not in this project: {_currentPath} (project path: {Application.dataPath})");
			return;
		}
		...
		else
		{
			...
			for (int folderIdx = startOfFolderIdx; folderIdx < _currentPathParts.Length; ++folderIdx)
			{
				...
				else if (GUILayout.Button(_currentPathPartsDisplayed[folderIdx], folderStyleToUse))
				{
					...
					Debug.Log($"Clicked path button, going to idx: {folderIdx} num: {_currentPathParts.Length}\n{folderToMoveTo}");
		...
		if (IsBrowserTypeForFiles)
		{
			if (IsAnimationVirtualFolderAnFbxFile(_currentPath, _files[idxOfSelectedItem]))
			{
				...
				else
				{
					...
					if (subAssets == null || subAssets.Length == 0)
					{
						Debug.LogWarning($"FileBrowser.SwitchCurrentPath(): no sub-assets inside of \"{_files[idxOfSelectedItem]}\"");
						return;
					}
