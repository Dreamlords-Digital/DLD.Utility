...
public abstract class BaseTextDataIO : ITextDataIO
{
...
#if UNITY_EDITOR
	public (LoadResult result, string errorMessage) TryLoadFromAssets<T>(
		string fileAssetsPath, ref T resultingLoadedObject) where T : ITextData
	{
		return TryLoadFromLocal($"{FileUtil.ProjectPath}/{fileAssetsPath}", ref resultingLoadedObject);
	}
#endif

	public (LoadResult result, string errorMessage) TryLoadFromStreamingAssets<T>(
		string fileAssetsPath, ref T resultingLoadedObject) where T : ITextData
	{
	...
	public (LoadResult result, string errorMessage) TryLoadFromResources<T>(
		string filePath, ref T resultingLoadedObject) where T : ITextData
	{
	...
	public (LoadResult result, string errorMessage) TryLoadBinaryFromResources<T>(
		string filePath, ref T resultingLoadedObject) where T : IBinaryData
	{
		var textAsset = Resources.Load<TextAsset>(filePath);
