...
public interface IDependencyResolver
{
	...
	/// <param name="obj">
	///    the instance that will be accessible to others when
	///    <see cref="Resolve{T}"/> or <see cref="Get{T}"/> is called.
	/// </param>
	...
	///    <para>
	///       The instance returned is either something that was created before
	///       and re-used, or a newly created one (via the parameter-less constructor).
	///    </para>
	///    <para>
	///       This method is thread-safe. Provided the rest of your code follows the rules
	...
	///       it's guaranteed that you are the only user of the instance you receive from this method.
	///    </para>
	/// </summary>
	/// <typeparam name="T">data type of the instance to get</typeparam>
	/// <returns>
	///    A usable instance, either a previously created instance (that is
	///    now unused and ready for new usage), or a completely new instance
	///    (created via the parameter-less constructor).
	/// </returns>
	T GetFromPool<T>() where T : class, IPooled, new();

	/// <summary>
	///    <para>
	///       Return a new usable instance of the type specified, from a pool
	///       that belongs to the group identified via the string <see cref="id"/>
	///    </para>
	///    <para>
	///       The instance returned is either something that was created before
	///       and re-used, or a newly created one (via the default constructor).
	///    </para>
	///    <para>
	///       This method is thread-safe. Provided the rest of your code follows the rules
	///       (call <see cref="IPooled.Dispose"/> on an <see cref="IPooled"/> object once finished using it, and
	///       do not keep using it after calling <see cref="IPooled.Dispose"/>), it's guaranteed
	///       that you are the only user of the instance you receive from this method.
	///    </para>
	/// </summary>
	/// <param name="id">
	///    ID of the pool to get from. Use this to create separate pools for
	///    instances even if their type is the same.
	/// </param>
	/// <typeparam name="T">data type of the instance to get</typeparam>
	/// <returns>
	///    A usable instance, either a previously created instance (that is
	...
	///    <para>
	///       Use this overload if you want the object's instantiation to use
	///       a constructor that has parameters.
	///    </para>
	///    <para>
	///       The instance returned is either a previously created instance (that is
	///       now unused and ready for new usage), or a completely new instance
	///       created via the supplied <see cref="System.Func{T}"/> callback.
	///    </para>
	///    <para>
	///       This method is thread-safe. Provided the rest of your code follows the rules
	///       (call <see cref="IPooled.Dispose"/> on an <see cref="IPooled"/> object once finished using it, and
	...
	/// <summary>
	///    <para>
	///       Return a ready-to-use instance of the type specified, from a pool.
	///       The instance returned is either something that was created before
	///       and re-used, or a newly created one.
	///    </para>
	///    <para>
	///       Use this overload if what you have is a System.Type,
	///       or you want to instantiate a concrete type,
	///       but get an interface in return.
	///    </para>
	///    <para>
	///       This method is thread-safe. Provided the rest of your code follows the rules
	///       (call <see cref="IPooled.Dispose"/> on an <see cref="IPooled"/> object once finished using it, and
	...
	/// <returns></returns>
	T GetFromPool<T>(Type typeToGet) where T : class, IPooled, new();
